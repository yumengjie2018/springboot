...
import static com.boco.jlappservice.utility.VeDate.strToDate;
...
public class DateUtils {
    ...
    public static final String DATE_PATTERN1 = "yyyy-MM-01";
    ...
    /**
     * @param date        如果为Null，则为当前月份1号
     * @param months        加减月数
     * @param includeTime 是否包括时分秒,true表示包含
     **/
    public static Date monthAdd(Date date, int months, boolean includeTime) throws ParseException {
        if (date == null) {
            date = new Date();
        }
        if (!includeTime) {
            SimpleDateFormat sdf = new SimpleDateFormat(DateUtils.DATE_PATTERN1);
            date = sdf.parse(sdf.format(date));
        }
        Calendar cal = Calendar.getInstance();
        cal.setTime(date);
    ...
    /**
     * 将Date转成yyyy-MM-dd HH:mm:ss格式字符串
     *
     * @param date 时间
     * @return String
     */
     ...
     * @throws ParseException
     */
    public static Date dateParseNewDateWhenException(String dateTimeString, String pattern) {
        try {
            if (StringUtils.isBlank(pattern)) {
                pattern = DateUtils.DATE_PATTERN;
            }
    ...
    /**
     * 获取相差天数的yyyy-MM-dd的日期
     *
     * @param curDate
     * @return
     */
    public static Date getTheDayBefore(Date curDate, int dateInt) {
        SimpleDateFormat sdf = new SimpleDateFormat(DATE_PATTERN);
        Calendar c = Calendar.getInstance();
        c.setTime(curDate);
        int day = c.get(Calendar.DATE);
        c.set(Calendar.DATE, day + dateInt);
        String strDayBefore = sdf.format(c.getTime());
        Date dayBefore = null;
        try {
            dayBefore = sdf.parse(strDayBefore);
     ...
     * @return
     */
    public static Date getTheDayBefore(Date curDate) {
        Calendar c = Calendar.getInstance();
        c.setTime(curDate);
        int day = c.get(Calendar.DATE);
        c.set(Calendar.DATE, day - 1);
        String strDayBefore = new SimpleDateFormat("yyyy-MM-dd").format(c.getTime());
        Date dayBefore = null;
        try {
            dayBefore = new SimpleDateFormat("yy-MM-dd").parse(strDayBefore);
        } catch (ParseException e) {
            e.printStackTrace();
        }
        return dayBefore;
     ...
     */
    public static String getNextDay(String nowdate, String delay,String timeformat) {
        try {
